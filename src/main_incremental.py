...
def main(argv=None):
    ...
    # gridsearch args
    parser.add_argument('--gridsearch-tasks', default=2, type=int,
                        help='Number of tasks to apply GridSearch (-1: all tasks) (default=%(default)s)')

    # Args -- Incremental Learning Framework
    args, extra_args = parser.parse_known_args(argv)
    args.results_path = os.path.expanduser(args.results_path)
    # args.nepochs replace with 50
    base_kwargs = dict(nepochs=50, lr=args.lr, lr_min=args.lr_min, lr_factor=args.lr_factor,
    ...
    utils.seed_everything(seed=args.seed)
    net = LLL_Net(init_model, args.num_tasks, remove_existing_head=not args.keep_existing_head, use_space_decoupling=True)
    utils.seed_everything(seed=args.seed)
